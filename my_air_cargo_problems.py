...
class AirCargoProblem(Problem):
    ...
    def actions(self, state: str) -> list:
        ...
        """
        # TODO implement
        # Reference the result() function in example_have_cake.py
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        # Based on the idea of goal_test(). For each clause from goals that is not in the clauses, it counts 1. 

        
        kb = PropKB()
        kb.tell(decode_state(node.state, self.state_map).pos_sentence())

        # Reference the goal_test() function.
        count = 0
        for clause in self.goal:
            if clause not in kb.clauses:
                count += 1
