...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Add an empty list in the end of list a_levels
        self.a_levels.append(list())
        for a in self.all_actions:
            # Build a new action node
            action = PgNode_a(a)
            for literal in self.s_levels[level]:
                if literal in action.prenodes:
                    # Add action node in the children level of literal and literal node in the parent level of action
                    literal.children.add(action)
                    action.parents.add(literal)
                    # Add action node into the end of a_leves
                    self.a_levels[level].append(action)
    ...
    def add_literal_level(self, level):
        ...
        # Add an empty set in the end of list a_levels
        self.s_levels.append(set())
        for action in self.a_levels[level - 1]:
            for literal in action.effnodes:
                # Add action node in the parent level of literal and literal node in the children level of action
                literal.parents.add(action)
                action.children.add(literal)

                # Add non-repeated literal node into the end of s_levels.
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for fluent in node_a2.action.effect_add:
            if fluent in node_a1.action.effect_rem:
                is_also_possible = True

        return is_possible or is_also_possible
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        is_also_possible = False
        for fluent in node_a1.action.effect_add:
            if fluent in node_a2.action.precond_neg:
                is_possible = True
        ...
        for fluent in node_a2.action.effect_add:
            if fluent in node_a1.action.precond_neg:
                is_also_possible = True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for s1 in node_s1.parents:
            for s2 in node_s2.parents:
                if not (s1.is_mutex(s2) and s1 != s2):
                    is_possible = False
                    break
        return is_possible
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            level_sum += self.calculate_cost(goal)
        return level_sum

    def calculate_cost(self, goal) -> int:
        '''Calculate the cost of the goal
        '''
        for initial_level, node in enumerate(self.s_levels):
            if PgNode_s(goal, True) in node:
